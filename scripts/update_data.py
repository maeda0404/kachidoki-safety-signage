#!/usr/bin/env python3
"""勝どき安全サイネージ用データ生成。VPWS50を主データに利用する正式版。"""
import gzip
import json
import re
...
TARGET_CODES = {'130010'}  # 東京地方。VPWS50はこの単位で収録
TARGET_NAMES = {'東京地方'}

DEFAULT_WARNINGS = {
    'dry': False, 'thunder': False, 'thunderForecast': False,
    'heavyRain': False, 'wave': False, 'storm': False,
...
ALL_OFFICIAL_FLAGS = ('dry', 'thunder', 'heavyRain', 'wave', 'storm',
                      'stormSurge', 'stormSurgeAdvisory')
...
def request_bytes(url):
    ...
    req = urllib.request.Request(
        ...
        headers={'User-Agent': 'kachidoki-safety-signage/4.0',
...
def name_to_flags(name):
    ...
    if '暴風' in name and '警報' in name:
        flags.add('storm')
    if '波浪' in name and '警報' in name:
        flags.add('wave')
    if '高潮' in name:
        if '注意報' in name and '警報' not in name:
            flags.add('stormSurgeAdvisory')
        elif '警報' in name:
            flags.add('stormSurge')
    return flags
