...
def main():
 w=get(WEATHER);c=w['current'];h=w['hourly'];dy=w['daily'];t=c['time'];i=h['time'].index(t[:13]+':00') if t[:13]+':00' in h['time'] else 0;a={'dry':False,'thunder':False,'heavyRain':False,'landslide':False,'wave':False};err=None
 try:
  j=get(JMA);s=json.dumps(j,ensure_ascii=False);active=any(x in s for x in ('発表','継続','警報','注意報','危険')) and '解除' not in s;a={'dry':active and '乾燥' in s,'thunder':active and '雷' in s,'heavyRain':active and '大雨' in s,'landslide':active and '土砂' in s,'wave':active and '波浪' in s}
